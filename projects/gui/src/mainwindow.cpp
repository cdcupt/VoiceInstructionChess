...
{
    m_recognizeVoiceAct = new QAction(tr("&Voice"), this);
    m_recognizeVoiceAct->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_R));

    m_recognizeGestureAct = new QAction(tr("&Gesture"), this);
    m_recognizeGestureAct->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_V));
    ...
    m_showGameWallAct->setShortcut(QKeySequence(Qt::ControlModifier + Qt::ShiftModifier + Qt::Key_V));
    ...
    connect(m_recognizeVoiceAct, SIGNAL(triggered()), this, SLOT(recognizeVoiceAct()));
    connect(m_recognizeGestureAct, SIGNAL(triggered()), this, SLOT(recognizeGestureAct()));
...
void MainWindow::createMenus()
{
    ...
    m_command = menuBar()->addMenu(tr("&Command"));
    m_command->addAction(m_recognizeVoiceAct);
    m_command->addAction(m_recognizeGestureAct);
}
...
void MainWindow::addGame(ChessGame* game)
{
    ...
    if (m_tabs.size() >= 2){
        onTabCloseRequested(m_tabBar->currentIndex()-1);                                  //modify
...
void MainWindow::showAboutDialog()
{
    ...
    html += "<p>" + tr("Modified by cdcbb in May 2016 ") + "</p>";
    html += "<p>" + tr("The original: Ilari Pihlajisto and Arto Jonsson") + "</p>";
...
void MainWindow::recognizeVoiceAct()
{
    QString comand = "zsh";
    ...
    //qDebug() << string.append(result) ;
    QString sResult = string.append(result);
    QPointF src = m_gameViewer->getScene()->transformationSrcCoordinate(&sResult);
    QPointF dist = m_gameViewer->getScene()->transformationDistCoordinate(src, &sResult);
    m_gameViewer->getScene()->moveChess(src, dist);

    QString html;
    html += "<h3>" + QString("Result: ") + "</h3>";
    html += "<p>" + sResult  + "</p>";
    html += "<p>" + QString("%1 ").arg(src.x()) + QString("%1").arg(src.y()) +
                    QString(" x ") +
                    QString("%1 ").arg(dist.x()) + QString("%1").arg(dist.y()) + "</p>";

    //QMessageBox::about(this, tr("Recognition Complete"), html);
}

void MainWindow::recognizeGestureAct()
{
    QString comand = "zsh";
    QStringList arguments ;
    arguments << "./setup.sh";
