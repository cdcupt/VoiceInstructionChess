...
void BoardScene::mousePressEvent(QGraphicsSceneMouseEvent* event)
{
    ...
    if (piece == nullptr || event->button() != Qt::LeftButton)
        return;
    qDebug() << piece->pieceType().type() << QString("belong to") << piece->pieceType().side().symbol();
...
QPointF BoardScene::transformationSrcCoordinate(QString* src)
{
    const char* myChar = src->toStdString().c_str();
    qDebug() << QString("Src: ") << myChar;
    if(myChar[0]-48 == 5 || myChar[0]-48 == 6)
    {
        for(auto i=m_targets.begin(); i!=m_targets.end(); i++)
        {
            if(i.key()->pieceType().type() == myChar[0]-48)
                return i.key()->scenePos();
        }

    }
    else if(myChar[0]-48 == 1)
    {
        int pos_x = -175 + (myChar[1]-48-1)*(50);
        int pos_y = -225;
        for(int i=0; i<8; i++)
        {
            pos_y += 50;
            GraphicsPiece* piece = pieceAt(QPointF(pos_x,pos_y));
            if(m_targets.contains(piece) && piece->pieceType().type() == myChar[0]-48)
            {
                if(myChar[2] == 'F')
                {
                    if(piece->pieceType().side().symbol() == QString("w"))
                    {
                        if(pieceAt(QPointF(pos_x,pos_y-50)) == nullptr)
                            return piece->scenePos();
                    }
                    else
                    {
                        if(pieceAt(QPointF(pos_x,pos_y+50)) == nullptr)
                ...
                {
                    //if(pieceAt(QPointF(pos_x)))
                    if(piece->pieceType().side().symbol() == QString("w"))
                    {
                        if(-175-50 + (myChar[3]-48)*50 > piece->scenePos().x())
                        {
                            GraphicsPiece* new_piece = pieceAt(QPointF(pos_x+50,pos_y-50));
                            if(new_piece!=nullptr && new_piece->pieceType().side().symbol() == QString("b"))
                                return piece->scenePos();
                        }
                        else if(-175-50 + (myChar[3]-48)*50 < piece->scenePos().x())
                        {
                                GraphicsPiece* new_piece = pieceAt(QPointF(pos_x-50,pos_y-50));
                                if(new_piece!=nullptr && new_piece->pieceType().side().symbol() == QString("b"))
                                    return piece->scenePos();
                    ...
                    else
                    {
                        ...
                        {
                            GraphicsPiece* new_piece = pieceAt(QPointF(pos_x-50,pos_y+50));
                            if(new_piece!=nullptr && new_piece->pieceType().side().symbol() == QString("w"))
                                return piece->scenePos();
                        }
                        else if(-175-50 + (myChar[3]-48)*50 > piece->scenePos().x())
                        {
    ...
    else if(myChar[0]-48 == 2)
    {
        if(myChar[2] == 'F')
        {

            GraphicsPiece* piece = pieceAt(QPointF(-225+(myChar[3]-48)*50 - 50,
                                                   -225+(myChar[1]-48)*50 + 100));
            if(piece != nullptr && m_targets.contains(piece) && piece->pieceType().side().symbol() == QString("w")
                                                             && piece->pieceType().type() == 2)
            {
                return QPointF(-225+(myChar[3]-48)*50 - 50,
                               -225+(myChar[1]-48)*50 + 100);
            }
            else
            {
                piece = pieceAt(QPointF(-225+(myChar[3]-48)*50 + 50,
                                        -225+(myChar[1]-48)*50 + 100));
                if(piece != nullptr && m_targets.contains(piece) && piece->pieceType().side().symbol() == QString("w")
                                                                 && piece->pieceType().type() == 2)
                    return QPointF(-225+(myChar[3]-48)*50 + 50,
                ...
                else
                {
                    ...
                    if(piece != nullptr && m_targets.contains(piece) && piece->pieceType().side().symbol() == QString("b")
                                                                     && piece->pieceType().type() == 2)
                        return QPointF(-225+(myChar[3]-48)*50 - 50,
                                       -225+(myChar[1]-48)*50 - 100);
                    else
        ...
        {
            GraphicsPiece* piece = pieceAt(QPointF(-225+(myChar[3]-48)*50 - 100,
                                                   -225+(myChar[1]-48)*50 + 50));
            if(piece != nullptr && m_targets.contains(piece) && piece->pieceType().type() == 2)
                return QPointF(-225+(myChar[3]-48)*50 - 100,
                               -225+(myChar[1]-48)*50 + 50);
            else
            {
                piece = pieceAt(QPointF(-225+(myChar[3]-48)*50 + 100,
                                        -225+(myChar[1]-48)*50 + 50));
                if(piece != nullptr && m_targets.contains(piece) && piece->pieceType().type() == 2)
                    return QPointF(-225+(myChar[3]-48)*50 + 100,
                                   -225+(myChar[1]-48)*50 + 50);
    ...
    return QPointF(-175,-175);
}

QPointF BoardScene::transformationDistCoordinate(QPointF pos_src, QString* src)
{
    const char* myChar = src->toStdString().c_str();
    qDebug() << QString("Dist: ") << myChar;
    GraphicsPiece* piece = pieceAt(pos_src);
    if(myChar[0]-48 != 5 && myChar[0]-48 != 6)
    {
        //P all way
        if(myChar[0]-48 == 1)
        {
            ...
            {
                if(piece->pieceType().side().symbol() == QString("w"))
                    return QPointF(piece->scenePos().x(),piece->scenePos().y() - (myChar[3]-48)*50);
                else
                    return QPointF(piece->scenePos().x(),piece->scenePos().y() + (myChar[3]-48)*50);
            }
            if(myChar[2] == 'T')
            {
                ...
                {
                    if(-175-50 + (myChar[3]-48)*50 > piece->scenePos().x())
                        return QPointF(piece->scenePos().x() + 50,
                                       piece->scenePos().y() - 50);
                    else if(-175-50 + (myChar[3]-48)*50 < piece->scenePos().x())
                        return QPointF(piece->scenePos().x() - 50,
                                       piece->scenePos().y() - 50);
        ...
        {
            return QPointF(-225 + (myChar[3]-48)*50,
                           -225 + (myChar[1]-48)*50);
        }
        else if(myChar[0]-48 == 3)
        {
            ...
            {
                if(piece->pieceType().side().symbol() == QString("w"))
                    return QPointF(piece->scenePos().x() + (myChar[3]-48)*50,
                                   piece->scenePos().y() - (myChar[3]-48)*50);
                else
                    return QPointF(piece->scenePos().x() - (myChar[3]-48)*50,
                                   piece->scenePos().y() + (myChar[3]-48)*50);
            }
            ...
            if(myChar[2] == 'T')
            {
                ...
                {
                    if(-175-50 + (myChar[3]-48)*50 > piece->scenePos().x())
                        return QPointF(piece->scenePos().x() + fabs((myChar[3]-48-(piece->scenePos().x()+225)/50)*50),
                                       piece->scenePos().y() + fabs((myChar[3]-48-(piece->scenePos().x()+225)/50)*50));
                    else if(-175-50 + (myChar[3]-48)*50 < piece->scenePos().x())
                        return QPointF(piece->scenePos().x() - fabs((myChar[3]-48-(piece->scenePos().x()+225)/50)*50),
                                       piece->scenePos().y() - fabs((myChar[3]-48-(piece->scenePos().x()+225)/50)*50));
        ...
        else if(myChar[0]-48 == 4)
        {
            ...
            {
                if(piece->pieceType().side().symbol() == QString("w"))
                    return QPointF(piece->scenePos().x(),
                                   piece->scenePos().y() - (myChar[3]-48)*50);
                else
                    return QPointF(piece->scenePos().x(),
                                   piece->scenePos().y() + (myChar[3]-48)*50);
...
void BoardScene::moveChess(QPointF src, QPointF dist)
{
    qDebug() << QString("I am in1");
    stopAnimation();

    GraphicsPiece* piece = pieceAt(src);
    if (!m_targets.contains(piece))
        return;
    if (piece == nullptr)
    {
    ...
    if (piece != nullptr)
    {
        QPointF targetPos(m_squares->mapFromScene(dist));
        tryMove(piece, targetPos);
    }
